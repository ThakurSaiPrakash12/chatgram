...
function Sidebar({ chats, setCurrentChat, user, refreshChats, loading: chatsLoading }) {
  ...
  const handleChatClick = (chat) => {
    if (selectedChat) {
      // If a chat is selected, deselect it instead of opening
  ...
  return (
    <div className="w-full h-full bg-gradient-to-b from-blue-50 via-purple-50 to-pink-50 dark:from-gray-800 dark:via-gray-850 dark:to-gray-900 p-3 md:p-5 border-r dark:border-gray-700 flex flex-col shadow-2xl transition-colors duration-200">
      ...
      {/* Profile and Delete Account Buttons */}
      <div className="mb-3 md:mb-5 flex gap-2">
        <button
          onClick={() => navigate("/profile")}
          className="flex-1 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-3 md:px-5 py-2 md:py-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-600 font-bold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 border border-purple-200 dark:border-gray-600 text-sm md:text-base"
        >
          <span className="text-lg md:text-xl">👤</span>
        ...
        <button
          ...
          className="bg-red-600 hover:bg-red-700 text-white px-3 md:px-4 py-2 md:py-3 rounded-xl font-bold shadow-md hover:shadow-lg transition-all flex items-center justify-center text-sm md:text-base"
      ...
      {selectedChat && (
        <div className="mb-3 p-3 bg-blue-100 dark:bg-blue-900/30 border-2 border-blue-500 dark:border-blue-400 rounded-xl flex items-center justify-between animate-fadeIn">
          <div className="flex items-center gap-2">
            <span className="text-lg">✓</span>
            <span className="text-sm font-semibold text-blue-700 dark:text-blue-300">Chat selected</span>
          </div>
          <button
            onClick={() => setSelectedChat(null)}
            className="text-sm font-medium text-blue-700 dark:text-blue-300 hover:text-blue-900 dark:hover:text-blue-100 px-3 py-1 hover:bg-blue-200 dark:hover:bg-blue-800/50 rounded-lg transition-all"
          >
            Cancel
          </button>
        </div>
      ...
      <ul className="flex-1 overflow-y-auto space-y-2">
        {chatsLoading && chats.length === 0 ? (
          ...
          return (
            <li
              key={chat._id}
              className={`p-2 md:p-3 rounded-xl cursor-pointer flex items-center gap-2 md:gap-3 transition-all duration-200 hover:shadow-md ${
                selectedChat === chat._id
                  ? 'bg-blue-100 dark:bg-blue-900/30 border-2 border-blue-500 dark:border-blue-400'
                  : 'bg-gray-50 dark:bg-gray-800 hover:bg-white dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-500'
              }`}
            ...
            >
              <div 
                ...
                onClick={(e) => {
                  ...
                  if (!selectedChat && !chat?.isGroupChat && chatPartner) {
                    e.stopPropagation();
              ...
              >
                <img
                  src={chatImage}
                  alt={chatName}
                  className={`w-12 h-12 md:w-14 md:h-14 rounded-full object-cover border-2 md:border-3 border-white dark:border-gray-700 shadow-md transition-transform ${!chat?.isGroupChat && !selectedChat ? 'hover:scale-110 cursor-pointer' : ''}`}
              ...
              <div className="flex-1 min-w-0 flex items-center gap-2">
                <div className="flex-1 min-w-0" onClick={() => handleChatClick(chat)}>
                ...
                {selectedChat === chat._id && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      deleteChat(chat._id, chatName);
                      setSelectedChat(null);
                    }}
                    disabled={deletingChat === chat._id}
                    className="p-2 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all disabled:opacity-50 animate-fadeIn"
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-96 shadow-2xl transform transition-all">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 dark:focus:ring-red-400 focus:border-transparent transition-all bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:bg-gray-300 dark:disabled:bg-gray-600 font-medium shadow-md hover:shadow-lg transition-all flex items-center gap-2"
              >
